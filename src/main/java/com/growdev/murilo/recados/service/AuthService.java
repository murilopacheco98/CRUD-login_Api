...
public class AuthService {
    ...
    public void authenticate(String token) {
        if (Objects.isNull(token)) throw new AutheticationFailExeception("token not present");

        if (Objects.isNull(getUser(token))) throw new AutheticationFailExeception("token not valid");
    }
}
